...
"""Batch Predictions Page for Credit Scoring Dashboard V3.
...
# Data directories (for sample templates)
...
def render_batch_predictions_page():
    ...
    # Use radio button for sub-navigation instead of tabs to avoid nesting issues
    mode = st.radio("Mode", ["Upload & Predict", "Download Reports"], horizontal=True, label_visibility="collapsed")
    st.markdown("---")

    if mode == "Upload & Predict":
        render_upload_tab()
    else:
...
def render_multi_file_upload():
        ...
        if uploaded_files:
            ...
            for name in ['application', 'bureau', 'bureau_balance', 'previous_application',
                ...
                if required_files[name]:
                    st.success(f"✅ **{name}** matched: `{required_files[name].name}`")
                else:
    ...
    if uploaded_count == 7:
        ...
        try:
            ...
            required_files['application'].seek(0) # Reset pointer after reading
...
def display_batch_results(result: dict, batch_name: str):
    ...
    st.write("DEBUG: API Result:", result) # Add debug print
