"""Model Performance Page for Credit Scoring Dashboard V3."""
...
def fetch_metrics_from_api():
    ...
    try:
        ...
        st.error("⏱️ API request timed out while fetching metrics.")
        return None
...
def plot_confusion_matrix(cm):
    ...
    for i in range(2):
        for j in range(2):
            # Display both row % and column % (Row % / (Col %))
    ...
    ax.set_title('Row Percentage / (Column Percentage)', fontsize=12, fontweight='bold')
    plt.tight_layout()
    return fig
...
def render_model_performance_page():
    ...
    logger.info("Rendering Model Performance page (V3)")
