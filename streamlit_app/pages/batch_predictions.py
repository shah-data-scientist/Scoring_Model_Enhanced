...
def render_upload_tab():
    ...
    st.markdown("### Upload Raw CSV Files")
    ...
    # Direct multi-file upload (no mode selection)
        ...
        if uploaded_files:
            ...
            for name in ['application', 'bureau', 'bureau_balance', 'previous_application', 
                ...
                if required_files[name]:
                    st.markdown(f"[OK] **{name}.csv** - {required_files[name].name}")
                else:
                    st.markdown(f"[  ] {name}.csv - *not uploaded*")
        else:
    ...
    if uploaded_count == 7:
        ...
        try:
            ...
            batch_name = st.text_input(
                ...
                help="Give this batch a name for easy identification"
            )
...
def display_batch_results(result: dict, batch_name: str):
    ...
    with col1:
        st.metric("Total Processed", result.get('n_predictions', result.get('total_processed', 0)))
    with col2:
        st.metric("Processing Time", f"{result.get('processing_time_seconds', 0):.2f}s")
    with col3:
        avg_prob = result.get('average_probability', 0)
        st.metric("Avg Probability", f"{avg_prob:.1%}" if avg_prob else "N/A")
    
    # Risk distribution (3 levels only)
    st.markdown("### Risk Distribution")
    risk_dist = result.get('risk_distribution', {})
    # Combine CRITICAL into HIGH if present
                ...
                for batch in batches:
                    ...
                    with st.expander(f"{status_text} {batch['batch_name']} (ID: {batch['id']})", expanded=False):
                        ...
                        with col2:
                            ...
                            st.write(f"**Processing Time:** {batch.get('processing_time_seconds', 0):.2f}s")
                        ...
                        with col_html:
                            ...
                            # Generate HTML directly and provide link
                            ...
                            if html_report:
                                ...
                                st.text_input(
                                    ...
                                    value=href[:100] + "...",
                                    key=f"url_{batch['id']}",
                                    help="This is a data URL - open in new tab to access"
                                )
            else:
